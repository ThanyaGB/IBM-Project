...
Two renderers, best first:

1. ``card_renderer`` — HTML rendered by headless Chromium.  This is the one to
   use: a browser shapes Devanagari and Kannada correctly (reordering vowel
   signs, ligatures), which Pillow only does when built against Raqm.
2. ``_render_with_pillow`` — the original Pillow path, kept as a fallback for
   machines with no browser.  It renders **English text only**, deliberately:
   drawing unshaped Devanagari produces a card full of boxes and misplaced
   vowel signs, which is worse than an English card.

Cards are saved to ./static/cards/ and the file path is returned.  The caller
is responsible for making the image reach the user — the WhatsApp channel
uploads it and references it by media id, so no public URL is needed.

If Pillow is not installed or any rendering step fails, the exception
propagates and app.py catches it gracefully (card generation is non-fatal).
"""
...
from bot import card_renderer
from bot.language import normalise_language

logger = logging.getLogger(__name__)

# Emoji are not present in Arial/DejaVu, so the Pillow path would draw them as
# tofu boxes.  The Chromium path renders them fine.
...
def generate_myth_card(record: dict, language: str = "en") -> str:
    """Render a branded myth/fact PNG card and return its absolute path.
    ...
    language : str
        The user's language.  With a Chromium renderer this produces a
        bilingual card whenever a *reviewed* translation exists; a draft
        translation is never printed (see retrieval.localised_text).

    Raises
    ------
    RuntimeError  — when neither renderer could produce an image.
    """
    language = normalise_language(language)
    try:
        return card_renderer.render_card(record, language)
    except Exception as exc:  # pylint: disable=broad-except
        logger.warning(
            "Chromium card rendering unavailable (%s) — falling back to Pillow",
            exc,
        )
    return _render_with_pillow(record)


def _render_with_pillow(record: dict) -> str:
    """Pillow fallback.  English text only — see the module docstring."""
    from PIL import Image, ImageDraw  # noqa: PLC0415

    record = {
        **record,
        "topic": _strip_emoji(record.get("topic", "")),
        "common_myth": _strip_emoji(record.get("common_myth", "")),
        "verified_fact": _strip_emoji(record.get("verified_fact", "")),
    }
    ...
    source_text = f"Source: {record.get('source', '')[:90]}"
